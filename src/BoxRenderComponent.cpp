...
BoxRenderComponent::BoxRenderComponent(ID3D11VertexShader* vertShaderPtr, ID3D11PixelShader* pixelShaderPtr)
	:
	...
	mpPixelShader(pixelShaderPtr)
{
	...
	bd.ByteWidth = sizeof(Vertex) * (UINT)boxMesh.Vertices.size();
	...
	ibd.ByteWidth = sizeof(UINT) * (UINT)boxMesh.Indices.size();
